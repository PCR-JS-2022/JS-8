...
function getUsefulInfo(letters) {
  letters.forEach(letter => {
    if (/встреча/gi.exec(letter.topic)) {
      ...
      letter.usefulInfo = letter.message.match(/(?<![А-ЯЁа-яёA-Za-z0-9])([A-ZА-ЯЁ]\d{3}[A-ZА-ЯЁ]{2}\s([0-9]{2,3}))(?![А-ЯЁа-яёA-Za-z0-9])/gmi)
        ? letter.message.match(/(?<![А-ЯЁа-яёA-Za-z0-9])([A-ZА-ЯЁ]\d{3}[A-ZА-ЯЁ]{2}\s([0-9]{2,3}))(?![А-ЯЁа-яёA-Za-z0-9])/gmi)[0]
        : null;
    } else if (/оплата/gi.exec(letter.topic)) {
      letter.usefulInfo = letter.message.match(/(([1-9]\d{0,2}(,\d{3})*|0|[1-9]\d*)([.]\d{2})?) р\./mi)
        ? parseFloat(letter.message.match(/(([1-9]\d{0,2}(,\d{3})*|0|[1-9]\d*)([.]\d{2})?) р\./mi)[0].trim().replace(/,/g, ''))
        : null;
    } else {
