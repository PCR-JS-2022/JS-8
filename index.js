...
function getUsefulInfo(letters) {
  letters.forEach(letter => {
    if (/встреча/gi.exec(letter.topic)) {
      ...
      letter.usefulInfo = letter.message.match(/([1-9]\[0-9]{0,2}(,\[0-9]{3})*|0|[1-9][0-9]*)(\.[0-9]{2})?) р\./mi)
        ? parseFloat(letter.message.match(/([1-9]\[0-9]{0,2}(,\[0-9]{3})*|0|[1-9][0-9]*)(\.[0-9]{2})?) р\./mi)[0].trim().replace(/,/g, ''))
        : null;
    } else {
