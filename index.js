...
function getUsefulInfo(letters) {
  letters.forEach(letter => {
    if (/встреча/gi.exec(letter.topic)) {
      ...
      letter.usefulInfo = letter.message.match(/(ИП|ООО|ОАО|АО|ЗАО)\s\".+?"/)[0].toString();
    } else if (/автомобиль/gi.exec(letter.topic)) {
      letter.usefulInfo = letter.message.match(/([A-ZА-Я]\d{3}[A-ZА-Я]{2}\s([0-9]{2,3}))+(?!\d)/)[0];
    } else if (/оплата/gi.exec(letter.topic)) {
      letter.usefulInfo = parseFloat(letter.message.match(/([\d\.,\s-]+[\d\.]+)/)[0].trim().replace(/,/g, ''));
    } else {
